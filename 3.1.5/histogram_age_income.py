...
####
ages = [] # Aggregator - lists onto which we add info
incomes = []

# Line is a walker - stores an item from the list
for line in data[3:]: # Omit 0, 1, 2 because they are not data
...
# make_PLTW_style(ax) # Makes the graph look suck
plt.show()

# print (sum(incomes)) Have no why they did this XD
# Histogram 2
fig_income, ax  = plt.subplots(1, 1)
ax.hist(incomes, color='#7777ff', bins=200)
ax.set_xlim(-5000, 1000000)
