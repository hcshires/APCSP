...
####
root = Tkinter.Tk()
root.wm_title('Adobe Photoshop CC 2019')
...
# Instantiate two radiobuttons
data = Tkinter.IntVar()

for i in range(len(MODE)):
	buttons[i] = Tkinter.Radiobutton(root, text=MODE[i], variable=data, value=i)
...
def up(event):
	...
	d = data.get()
	if d == 0:
		new_shape = canvas.create_oval(startx-r, starty-r, startx+r, starty+r, fill=tk_color_string, outline='#000000')
		shapes.append(new_shape) # aggregate the canvas' item
	else:
		new_shape = canvas.create_rectangle(startx-r, starty-r, startx+r, starty+r, fill=tk_color_string, outline='#000000')
		shapes.append(new_shape) # aggregate the canvas' item
